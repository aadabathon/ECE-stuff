module PID(clk,rst_n,error,drv_duty);
  ...
  localparam D_coeff = 32;		// in the range 0 to 64
 ...
 //Steady state error with just P_coeff @ 4 -> 256
 //Steady state error with just P_coeff @ 8 -> 170
 //Now we add an I_coeff = 1 -> error sharply drops off until reaching a steady state of 0
 //Now we add a D_coeff = 32 -> Way choppier and rigid analog curves, but it reaches steady state error of 0 much quicker
